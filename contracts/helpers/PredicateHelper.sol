...
contract PredicateHelper {
    ...
    function or(uint256 offsets, bytes calldata data) external view returns(bool) {
        ...
        for (uint256 i = 0; (current = uint32(offsets >> (i << 5))) != 0; i++) {
            (bool success, uint256 res) = Callib.compactStaticcallForUint(address(this), data[previous:current]);
            if (success && res == 1) {
                return true;
            }
            previous = current;
    ...
    function eq(uint256 value, bytes calldata data) external view returns(bool) {
        (bool success, uint256 res) = Callib.compactStaticcallForUint(address(this), data);
