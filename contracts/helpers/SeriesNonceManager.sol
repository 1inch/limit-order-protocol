...
contract SeriesNonceManager {
    ...
    function nonce(address maker, uint96 series) public view returns(uint256) {
        return _nonces[uint160(maker) | (uint256(series) << 160)];
    }

    /// @notice Advances nonce by one
    function increaseNonce(uint96 series) external {
        advanceNonce(series, 1);
    }

    /// @notice Advances nonce by specified amount
    function advanceNonce(uint96 series, uint256 amount) public {
    ...
    function nonceEquals(address maker, uint256 series, uint256 makerNonce) public view returns(bool) {
        return _nonces[uint160(maker) | (uint256(series) << 160)] == makerNonce;
    }
}
