...
const { fillWithMakingAmount, signOrder, buildOrder, buildMakerTraits, compactSignature } = require('./helpers/orderUtils');
...
describe('MeasureGas', function () {
    ...
    it('swap without predicates', async function () {
        ...
        const { r, vs } = compactSignature(await signOrder(order, chainId, await swap.getAddress(), addrs[1]));
