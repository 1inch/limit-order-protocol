...
const { buildOrder, signOrder, makeMakingAmount, compactSignature, buildOrderRFQ, signOrderRFQ } = require('./helpers/orderUtils');
...
describe('Dutch auction', function () {
    ...
    it('swap with takingAmount 50% time passed', async function () {
        ...
        const { r, vs } = compactSignature(signature);
        await swap.connect(addr1).fillOrderRFQExt(order, r, vs, ether('0.075'), ether('100'), order.extension);
    ...
    it('swap with takingAmount 0% time passed', async function () {
        ...
        const { r, vs } = compactSignature(signature);
        await swap.connect(addr1).fillOrderRFQExt(order, r, vs, ether('0.1'), ether('100'), order.extension);
    ...
    it('swap with takingAmount 100% time passed', async function () {
        ...
        const { r, vs } = compactSignature(signature);
        await swap.connect(addr1).fillOrderRFQExt(order, r, vs, ether('0.05'), ether('100'), order.extension);
