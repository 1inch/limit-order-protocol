...
describe('LimitOrderProtocol', function () {
    ...
    describe('wip', function () {
        ...
        it('should floor maker amount', async function () {
            ...
            expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
            expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-9, 9]);
        });
        ...
        it('should ceil taker amount', async function () {
            ...
            expect(tx).to.changeTokenBalances(dai, [addr, addr1], [4, -4]);
            expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
        });
        ...
        it('ERC721Proxy should work', async function () {
            ...
            expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-10, 10]);
        });
    });
    ...
    describe('MakerTraits', function () {
        ...
        it('Allow taker rate improvement', async function () {
            ...
            expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-3, 3]);
        });
    });
    ...
    describe('ETH Maker Orders', function () {
        ...
        it('Partial fill', async function () {
            ...
            expect(tx).to.changeEtherBalance(addr1, ether('-0.3'));
            expect(tx).to.changeTokenBalance(weth, ethOrders, ether('0.3'));
            ...
            expect(tx2).to.changeTokenBalances(dai, [addr, ethOrders, addr1], [ether('-200'), '0', ether('200')]);
            expect(tx2).to.changeTokenBalances(weth, [addr, ethOrders, addr1], [ether('0.2'), ether('-0.2'), '0']);

            /// Remaining fill
            const tx3 = await swap.fillContractOrderExt(order, signature, ether('100'), ether('0.1'), addr.address, '0x', '0x', order.extension);
            expect(tx3).to.changeTokenBalances(dai, [addr, ethOrders, addr1], [ether('-100'), '0', ether('100')]);
            expect(tx3).to.changeTokenBalances(weth, [addr, ethOrders, addr1], [ether('0.1'), ether('-0.1'), '0']);
        ...
        it('Partial fill -> cancel -> refund maker -> fail to fill', async function () {
            ...
            expect(tx3).to.changeTokenBalance(weth, ethOrders, ether('-0.1'));
            expect(tx3).to.changeEtherBalance(addr1, ether('0.1'));
    ...
    describe('Predicate', function () {
        ...
        it.skip('`or` should pass', async function () {
            ...
            expect(tx).to.changeTokenBalances(dai, [addr.address, addr1.address], [1, -1]);
            expect(tx).to.changeTokenBalances(weth, [addr.address, addr1.address], [-1, 1]);
        });
    ...
    describe('Expiration', function () {
        ...
        it('should fill partially if not enough coins (taker)', async function () {
            ...
            expect(tx).to.changeTokenBalances(dai, [addr, addr1], [2, -2]);
            expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-2, 2]);
        });
    ...
    describe('Range limit orders', function () {
        ...
        async function fillRangeLimitOrder (makerAsset, takerAsset, fillOrderParams, isByMakerAsset, swap, rangeAmountCalculator, chainId) {
            ...
            expect(tx).to.changeTokenBalances(takerAsset.asset, [maker, taker], [
                isByMakerAsset ? rangeAmount1 : takerAsset.ether(fillOrderParams[0].takingAmount),
                isByMakerAsset ? -rangeAmount1 : -takerAsset.ether(fillOrderParams[0].takingAmount)]);
            expect(tx).to.changeTokenBalances(makerAsset.asset, [maker, taker], [
            ...
            expect(tx2).to.changeTokenBalances(takerAsset.asset, [maker, taker], [
                isByMakerAsset ? rangeAmount2 : takerAsset.ether(fillOrderParams[1].takingAmount),
                isByMakerAsset ? -rangeAmount2 : -takerAsset.ether(fillOrderParams[1].takingAmount),
            ]);
            expect(tx2).to.changeTokenBalances(makerAsset.asset, [maker, taker], [
