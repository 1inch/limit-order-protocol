...
describe('LimitOrderProtocol', function () {
    ...
    describe('ETH Maker Orders', function () {
        ...
        it('Partial fill', async function () {
            ...
            await ethOrders.ethOrderDeposit(order, { value: ether('0.3') });
            ...
            /// Partial fill
            await swap.connect(addr1).fillOrder(order, signature, '0x', 0, ether('200'), ether('0.2'));
            /// Remaining fill
            await swap.connect(addr1).fillOrder(order, signature, '0x', 0, ether('100'), ether('0.1'));
        ...
        it('Partial fill -> cancel -> refund maker -> fail to fill', async function () {
            ...
            /// Cancel order
            addreth = await ethers.provider.getBalance(addr.address);
            await ethOrders.cancelOrder(order);
            assertRoughlyEqualValues(await ethers.provider.getBalance(addr.address), addreth.add(ether('0.1')), 1e-2);
            /// Remaining fill
            await expect(swap.connect(addr1).fillOrder(order, signature, '0x', 0, ether('100'), ether('0.1'))).to.be.revertedWithCustomError(swap, 'RemainingAmountIsZero');
        });
        ...
        it('Invalid order', async function () {
            ...
            await expect(ethOrders.ethOrderDeposit(order, { value: ether('0.3') })).to.be.revertedWithCustomError(ethOrders, 'InvalidOrder');
        });
    });
