...
describe('LimitOrderProtocol', function () {
    ...
    describe('wip', function () {
        ...
        it('should floor maker amount', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 9, 1);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-9, 9]);
        });
        ...
        it('should ceil taker amount', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 4, fillWithMakingAmount(1));
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [4, -4]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
        });
        ...
        it('should unwrap weth', async function () {
            ...
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [ether('-5'), ether('5')]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [ether('0'), ether('-1')]);
            await expect(tx).to.changeEtherBalance(addr, ether('1'));
        });
        ...
        it('ERC721Proxy should work', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrderExt(order, r, vs, 10, fillWithMakingAmount(10), order.extension);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-10, 10]);
        });
    });
    ...
    describe('MakerTraits', function () {
        ...
        it('unwrap weth for maker', async function () {
            ...
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            await expect(tx).to.changeTokenBalance(weth, addr, -2);
            await expect(tx).to.changeEtherBalance(addr1, 2);
        });
        ...
        it('Allow taker rate improvement', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrderTo(order, r, vs, 10, 2, addr.address, takerIncreaser.address);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-3, 3]);
        });
    });
    ...
    describe('Permit', function () {
        describe('fillOrderToWithPermit', function () {
            ...
            it('DAI => WETH', async function () {
                ...
                const tx = await swap.fillOrderToWithPermit(order, r, vs, 1, fillWithMakingAmount(1), addr.address, '0x', permit);
                await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
                await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
            });
        ...
        describe('maker permit', function () {
            ...
            it('maker permit works', async function () {
                ...
                const tx = await swap.connect(addr1).fillOrderExt(order, r, vs, 1, fillWithMakingAmount(1), order.extension);
                await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
                await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
            });
            ...
            it('skips order permit flag', async function () {
                ...
                const tx = await swap.connect(addr1).fillOrderExt(order, r, vs, 1, skipMakerPermit(0), order.extension);
                await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
                await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
            });
        });
    ...
    describe('Amount Calculator', function () {
        ...
        it('empty takingAmountGetter should work on full fill', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 10, fillWithMakingAmount(10));
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-10, 10]);
        });
        ...
        it('empty makingAmountGetter should work on full fill', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 10, 10);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [10, -10]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-10, 10]);
        });
    });
    ...
    describe('ETH Maker Orders', function () {
        ...
        it('Partial fill', async function () {
            ...
            const tx = await ethOrders.connect(addr1).ethOrderDeposit(order, order.extension, { value: ether('0.3') });
            await expect(tx).to.changeEtherBalance(addr1, ether('-0.3'));
            await expect(tx).to.changeTokenBalance(weth, ethOrders, ether('0.3'));
            ...
            /// Partial fill
            const tx2 = await swap.fillContractOrderExt(order, signature, ether('200'), ether('0.2'), addr.address, '0x', '0x', order.extension);
            await expect(tx2).to.changeTokenBalances(dai, [addr, ethOrders, addr1], [ether('-200'), '0', ether('200')]);
            await expect(tx2).to.changeTokenBalances(weth, [addr, ethOrders, addr1], [ether('0.2'), ether('-0.2'), '0']);

            /// Remaining fill
            const tx3 = await swap.fillContractOrderExt(order, signature, ether('100'), ether('0.1'), addr.address, '0x', '0x', order.extension);
            await expect(tx3).to.changeTokenBalances(dai, [addr, ethOrders, addr1], [ether('-100'), '0', ether('100')]);
            await expect(tx3).to.changeTokenBalances(weth, [addr, ethOrders, addr1], [ether('0.1'), ether('-0.1'), '0']);
        ...
        it('Partial fill -> cancel -> refund maker -> fail to fill', async function () {
            ...
            /// Cancel order
            const tx3 = await ethOrders.connect(addr1).cancelOrder(order.makerTraits, orderHash);
            await expect(tx3).to.changeTokenBalance(weth, ethOrders, ether('-0.1'));
            await expect(tx3).to.changeEtherBalance(addr1, ether('0.1'));
    ...
    describe('Predicate', function () {
        ...
        it.skip('`or` should pass', async function () {
            ...
            const tx = await swap.fillOrderExt(order, r, vs, 1, fillWithMakingAmount(1), order.extension);
            await expect(tx).to.changeTokenBalances(dai, [addr.address, addr1.address], [1, -1]);
            await expect(tx).to.changeTokenBalances(weth, [addr.address, addr1.address], [-1, 1]);
        });
        ...
        it('`and` should pass', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, fillWithMakingAmount(1), 1);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [1, -1]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-1, 1]);
        });
    ...
    describe('Expiration', function () {
        ...
        it('should fill partially if not enough coins (taker)', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 3, 2);
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [2, -2]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-2, 2]);
        });
        ...
        it('should fill partially if not enough coins (maker)', async function () {
            ...
            const { r, vs } = compactSignature(await signOrder(order, chainId, swap.address, addr1));
            const tx = await swap.fillOrder(order, r, vs, 3, fillWithMakingAmount(3));
            await expect(tx).to.changeTokenBalances(dai, [addr, addr1], [2, -2]);
            await expect(tx).to.changeTokenBalances(weth, [addr, addr1], [-2, 2]);
        });
    });
    ...
    describe('ETH fill', function () {
        ...
        it('should fill with ETH', async function () {
            ...
            await expect(tx).to.changeEtherBalance(addr, -3);
        });
    ...
    describe('Range limit orders', function () {
        ...
        async function fillRangeLimitOrder (makerAsset, takerAsset, fillOrderParams, isByMakerAsset, swap, rangeAmountCalculator, chainId) {
            ...
            await expect(tx).to.changeTokenBalances(takerAsset.asset, [maker, taker], [
                isByMakerAsset ? rangeAmount1 : takerAsset.ether(fillOrderParams[0].takingAmount),
                isByMakerAsset ? -rangeAmount1 : -takerAsset.ether(fillOrderParams[0].takingAmount)]);
            await expect(tx).to.changeTokenBalances(makerAsset.asset, [maker, taker], [
            ...
            const rangeAmount2 = (await getRangeAmount(
                ...
                isByMakerAsset ? makingAmount - makerAsset.ether(fillOrderParams[0].makingAmount) : makingAmount - rangeAmount1,
            )).toBigInt();
            const tx2 = await swap.fillOrderExt(
                order,
                r,
                vs,
                isByMakerAsset
            ...
            await expect(tx2).to.changeTokenBalances(takerAsset.asset, [maker, taker], [
                isByMakerAsset ? rangeAmount2 : takerAsset.ether(fillOrderParams[1].takingAmount),
                isByMakerAsset ? -rangeAmount2 : -takerAsset.ether(fillOrderParams[1].takingAmount),
            ]);
            await expect(tx2).to.changeTokenBalances(makerAsset.asset, [maker, taker], [
                isByMakerAsset ? -makerAsset.ether(fillOrderParams[1].makingAmount) : -rangeAmount2,
                isByMakerAsset ? makerAsset.ether(fillOrderParams[1].makingAmount) : rangeAmount2,
            ]);
        };
