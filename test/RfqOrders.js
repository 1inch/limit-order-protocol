...
describe('RFQ Orders in LimitOrderProtocol', function () {
    ...
    describe('wip', function () {
        it('@skip-on-coverage should swap fully based on RFQ signature', async function () {
            ...
            for (const salt of ['000000000000000000000001', '000000000000000000000002']) {
                ...
                const receipt = await swap.fillOrderRFQ(order, r, vs, makingAmount(1));
    ...
    describe('Permit', function () {
        describe('fillOrderRFQToWithPermit', function () {
            it('DAI => WETH', async function () {
                ...
                const { r, vs } = compactSignature(signature);
                await swap.fillOrderRFQToWithPermit(order, r, vs, makingAmount(1), addr.address, emptyInteraction, permit);
            ...
            it('rejects reused signature', async function () {
                ...
                const permit = await getPermit(addr.address, addr, weth, '1', chainId, swap.address, '1');
                const { r, vs } = compactSignature(signature);
                const requestFunc = () => swap.fillOrderRFQToWithPermit(order, r, vs, takingAmount(1), addr.address, emptyInteraction, permit);
                await requestFunc();
    ...
    describe('OrderRFQ Cancelation', function () {
        ...
        it('should not fill cancelled order', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await expect(
                swap.fillOrderRFQ(order, r, vs, makingAmount(1)),
    ...
    describe('Expiration', function () {
        it('should fill RFQ order when not expired', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await swap.fillOrderRFQ(order, r, vs, makingAmount(1));
        ...
        it('should fill RFQ order when not expired (compact)', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await swap.fillOrderRFQ(order, r, vs, takingAmount(1));
        ...
        it('should not partial fill RFQ order when 0', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await expect(
                swap.fillOrderRFQ(order, r, vs, takingAmount(1)),
    ...
    describe('ETH fill', function () {
        it('should fill with ETH', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await swap.fillOrderRFQ(order, r, vs, takingAmount(3), { value: 3 });
        ...
        it('should reverted with takerAsset WETH and incorrect msg.value', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await expect(
                swap.fillOrderRFQ(order, r, vs, makingAmount(900), { value: 2 }),
            ).to.be.revertedWithCustomError(swap, 'InvalidMsgValue');
            await expect(
                swap.fillOrderRFQ(order, r, vs, makingAmount(900), { value: 4 }),
            ).to.be.revertedWithCustomError(swap, 'InvalidMsgValue');
        });
        ...
        it('should reverted with takerAsset non-WETH and msg.value greater than 0', async function () {
            ...
            const { r, vs } = compactSignature(signature);
            await expect(
                swap.fillOrderRFQ(order, r, vs, makingAmount(900), { value: 1 }),
            ).to.be.revertedWithCustomError(swap, 'InvalidMsgValue');
        });
    });
});
